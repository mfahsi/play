...
class TreePostOrderTest extends AnyFlatSpec with should.Matchers  {
  ...
  object TreeF:
      def postOrderPrint(tree:Node[T]): Unit = {
        ...
        stack.push(tree)
        rec(stack,done)
        def process(stack: mutable.Stack[Node[T]], done: mutable.Set[Node[T]]): Unit = {
          val pop = stack.pop()
          println(pop.v)
          done.addOne(pop)
          rec(stack, done)
        ...
        def rec(stack: mutable.Stack[Node[T]], done: mutable.Set[Node[T]]): Unit = {
         // println(s"stack ${stack.toList.map(_.v)}")
          if (stack.isEmpty) {
            ...
            if (top.isLeaf || (isDone(top.left,done) && isDone(top.right,done))) {
             ...
             // stack.push(top)
              if (!isDone(top.right,done)) {
                stack.push(top.right)
              }
              if (!isDone(top.left,done)) {
                stack.push(top.left)
              }
              rec(stack, done)
            }
  ...
  "post order tree" should "print"  in {
     val tree = Node[T](1,null,Node(2,null,Node(5,Node(3,null,Node(4,null,null)),Node(6,null,null))))
